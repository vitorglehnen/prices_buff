...
class ItemDao:
    ...
    def inserir(self, item):
        sql = f"""
                    ...
                    VALUES ({item.codigo},'{item.descricao}', {item.preco}, {item.quantidade}, '{item.inativo}')
                    """
        self.conexao_instance.query_insert(sql)
    ...
    def buscar(self, item):
        sql = f"""
                    ...
                    FROM public.item i
                    WHERE i.coditem = {item.codigo}
                    """
        dataset = self.conexao_instance.query_select(sql)

        if len(dataset) == 0:
            return Item(None, None, None, None, None)
        else:
            item.codigo = dataset[0][0]
            item.descricao = dataset[0][1]
            item.preco = dataset[0][2]
            item.quantidade = dataset[0][3]
            item.inativo = dataset[0][4]
    ...
    def buscar_todos(self):
        ...
        for registro in dataset:
            item = Item(registro[0], registro[1], registro[2], registro[3], registro[4])
            itens.append(item)
